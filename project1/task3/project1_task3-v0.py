...
#print("shape of psi: ", np.shape(psi)) # shape should be (10000, 500)
pi = psi - np.mean(psi, axis=0)
#print("shape of pi: ", np.shape(pi)) 
(_, s, V) = np.linalg.svd(pi)
S = np.diag(s)
#print("shape of S and V: ", np.shape(S), np.shape(V))
Z = np.matmul(pi, V)
